...
Three details are worth knowing if you extend this:
...
A design with no segment file leaves its hosts cabled but unaddressed, which the
demo task says out loud rather than inventing a subnet the fabric does not route.

**The generator never reallocates.** A host that already has a primary address
keeps it, so re-running the generator is safe, and a host addressed by hand is
left alone.
