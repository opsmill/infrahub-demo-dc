...
- `extended` runs when a pull request is labelled `full-integration`.
  `.github/workflows/ci.yml` selects the tier.

Label a pull request that changes a version, a generator or a transform, so those workflows are
exercised before it merges. The tier is opt-in rather than automatic on version bumps because an
upstream Infrahub fault currently stops it passing reliably: Infrahub intermittently fails to resolve
...
```bash
INFRAHUB_TESTING_IMAGE_VER=1.4.0 uv run pytest -m "not extended"
```
