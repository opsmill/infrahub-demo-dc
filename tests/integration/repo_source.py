...
EXCLUDED = (
    ...
    "clab-*",
    ".clab.*",
    # Local credentials. `.env` is not incidental clutter here: AGENTS.md documents it as required
    # and as holding INFRAHUB_API_TOKEN, and `GitRepo` *commits* this copy before Infrahub clones
    # it -- so without this the developer's token ends up in a git history inside the container.
    # Infrahub never reads it; only `invoke` and docker compose do, on the host.
    ".env",
    ".env.*",
    ".envrc",
    ".direnv",
