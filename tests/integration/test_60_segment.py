...
def segment_state() -> dict[str, Any]:
...
async def test_01_load_segment(
    ...
    segment_state["segment_id"] = segment.id
    segment_state["vlan_id"] = segment.vlan_id.value
    log.info("Segment %s attached to %s with VLAN %s", segment.id, c.DC_ARISTA_NAME, segment_state["vlan_id"])
...
async def test_02_trigger_rule_attaches_customer_interfaces(
    ...
    attached = await _segment_interface_ids(client, segment_id=segment_id, branch=BRANCH)
    segment_state["attached"] = attached
...
async def test_03_attached_interfaces_are_the_right_ones(
    ...
    attached: set[str] = segment_state["attached"]
...
async def test_04_segment_generator_rerun_is_idempotent(
    ...
    task = await h.run_generator(
        ...
        node_ids=[segment_state["segment_id"]],
        branch=BRANCH,
    )
    log.info("Explicit create_segment re-run finished in state %s", task.state)
    await h.wait_for_quiescence(client, branch=BRANCH, kinds=["InterfacePhysical", "ServiceNetworkSegment"])

    after = await _segment_interface_ids(client, segment_id=segment_state["segment_id"], branch=BRANCH)
    assert after == segment_state["attached"], (
        f"Re-running create_segment changed the attached interfaces: {len(segment_state['attached'])} -> {len(after)}"
    )
...
async def test_05_segment_merges_to_main(async_client_main: InfrahubClient, segment_state: dict[str, Any]) -> None:
    ...
    on_main = await _segment_interface_ids(client, segment_id=segment_state["segment_id"], branch="main")
    assert on_main == segment_state["attached"], (
        f"Segment interfaces differ after merge.\n"
        f"  On {BRANCH}: {len(segment_state['attached'])}\n  On main:  {len(on_main)}"
    )
