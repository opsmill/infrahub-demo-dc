...
def test_explicit_truthy_pull_wins_over_the_custom_build_default(value: str) -> None:
    ...
    Treating only the literal "true" as truthy would invert the caller's intent, and `as_env()` would
    then write the inversion back out — so a repository setting `=1` and merging `as_env()` into
    `os.environ` would overwrite its own value and run a stale local image while the suite reported the
    resolved tag.
    """
...
def test_unresolvable_tag_is_rejected_by_name() -> None:
    """Nothing resolving a tag must name its own cause, not surface as a broken iterator.

    Every candidate is empty here: no environment override, no ``default_tag``, and a
    ``packaged_version`` that an uninstalled or unreadable ``infrahub-testcontainers`` would produce.
    An undefaulted ``next()`` over that list raises ``StopIteration``, which reads as a bug in the
    resolver rather than as unresolvable configuration.
    """
