...
async def test_02_create_a_conflicting_change(
    ...
    conflict_state["device_id"] = on_branch.id
...
async def test_05_closing_a_proposed_change_leaves_main_alone(
    ...
    pc = await client.get(kind="CoreProposedChange", id=conflict_state["pc_id"])
    pc.state.value = "closed"
    await pc.save()

    reread = await client.get(kind="CoreProposedChange", id=conflict_state["pc_id"])
