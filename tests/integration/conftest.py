...
Sharing one deployment is only safe because ordering and prerequisites are declared rather than
assumed. Four things carry that, and a repository adopting session scope needs all four: module
filenames are numbered (``test_00_``, ``test_10_``, ...) and collection is never randomised;
``pytest-dependency`` marks declare what each step needs, so a test whose prerequisite did not run
skips instead of failing spuriously; ``pytest-order`` is installed alongside it; and
``asyncio_default_fixture_loop_scope`` is pinned in ``pyproject.toml``. A test that asserts something
is *absent* is safe only when it is a declared dependent of the state it assumes -- never because it
happened to collect early.

Named extension points
----------------------

This file is the canonical session-scoped bootstrap shared by the demo repositories. A sibling
repository adopting it attaches its needs at the named places below rather than forking the
file:

* :func:`stack_image` is the single image-choice override: pass ``repository=``,
  ``custom_build=True`` and optionally ``default_tag=`` to :func:`resolve_stack_image`, and merge
  its ``as_env()`` mapping into ``os.environ`` **at module import time** (see
  :mod:`tests.integration.stack_config` for why).
* A custom-image build gate attaches as a new autouse fixture beside
  :func:`bootstrapped_deployment` -- never by editing it -- so the two gates stay independently
  skippable.
* A client override attaches at :func:`client_main` / :func:`async_client_main` and stays
  **function-scoped**: an async client binds to the event loop of the test that first used it.
* The session-scoped chain ``stack_directory`` -> ``remote_repos_dir``/``remote_backups_dir`` ->
  :func:`infrahub_compose` -> :func:`infrahub_app` -> :func:`infrahub_address`, plus
  :func:`infrahub_bootstrap`, must not be re-scoped or forked.
"""
...
def module_state() -> dict[str, Any]:
    """State carried between a module's ordered steps.

    Module-scoped, so every workflow module gets its own instance.
...
def stack_image() -> StackImage:
    ...
    This repository runs vanilla Infrahub, so it passes no repository, no default tag and no
    ``custom_build`` -- the defaults are the vanilla case. A repository that builds its own image
    overrides those three here, in one visible place, which is the header block spec §5.1 fix 3 asks
    for.
...
def infrahub_version(stack_image: StackImage) -> str:
    ...
    Returns:
        The resolved tag -- an explicit override when one is set, otherwise the installed
...
@pytest.fixture(scope="session")
def stack_directory(tmp_path_factory: pytest.TempPathFactory, stack_image: StackImage) -> Generator[Path, None, None]:
    ...
    Args:
        ...
        stack_image: Image under test, reused for the ownership-reclaiming container.
    ...
    _reclaim_bind_mount_ownership(directory, image=stack_image.reference)
...
def infrahub_app(
    ...
    try:
        ...
        # A partial `up` still leaves containers behind, so tear them down -- but suppressed, because
        # the compose logs above are the diagnostic here and a secondary failure in `stop()` (docker
        # unreachable, say, which is the likeliest reason `start()` failed at all) must not replace
        # them with a less useful traceback.
    ...
    try:
        ...
        # The stack is shared, so this is the only chance to capture logs for the whole run.
        # Attaching them on any failure is what makes a CI-only failure diagnosable without a local
        # reproduction.
        if request.session.testsfailed:
            stdout, stderr = infrahub_compose.get_logs("infrahub-server", "task-worker")
            warnings.warn(f"Container logs:\nStdout:\n{stdout}\nStderr:\n{stderr}", stacklevel=2)
    finally:
        # `finally`, unlike the unguarded call this replaced (and unlike `TestInfrahubDocker`, which
        # this otherwise mirrors): at class scope a leak costs one class's containers, but at session
        # scope it strands the entire deployment -- Neo4j, RabbitMQ, Redis, both API servers, both
        # task workers, Prefect and its Postgres -- for the whole run. The realistic trigger is the
        # log dump above: `get_logs` names two services, so a testcontainers bump that renames or
        # drops either raises here, which is exactly the change this suite exists to validate.
        infrahub_compose.stop()
...
def infrahub_api_token() -> str:
    ...
    from the environment. The implicit route used to work in CI only because ``ci.yml`` happened to
    set that variable to the same literal that ``infrahub-testcontainers`` seeds the deployment with
    (it no longer sets it) -- so the suite silently failed to authenticate anywhere that variable was
    not exported, and would have broken just as silently if testcontainers ever changed its default
    token.
...
def repo_source_directory(stack_directory: Path) -> Path:
    ...
    return prepare_repo_source(
        root_directory=c.PROJECT_DIRECTORY,
...
def infrahub_bootstrap(
    ...
    """
    address = infrahub_address  # local name for the many call sites below
