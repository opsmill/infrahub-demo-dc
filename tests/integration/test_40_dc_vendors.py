...
async def test_01_load_cisco_design(
    ...
    vendor_state["topology_name"] = topology_name
    ...
    vendor_state["topology_id"] = topology.id
    vendor_state["design_name"] = topology.design.peer.name.value
    log.info("Loaded %s using design %s", topology_name, vendor_state["design_name"])
...
async def test_02_fabric_matches_design(
    ...
    counts = await h.wait_for_topology(
        async_client_main,
        topology_name=vendor_state["topology_name"],
        design_name=vendor_state["design_name"],
        branch=BRANCH,
    )
    vendor_state["role_counts"] = counts

    assert counts.get("border_leaf"), (
        f"Design {vendor_state['design_name']!r} calls for border leafs but none were created: {counts}"
    )
...
async def test_03_border_leafs_are_cabled_and_addressed(
    ...
    loopbacks = await client.filters(
        ...
        device__ids=[device.id for device in border_leafs],
        branch=BRANCH,
    )
    ...
    assert not missing, f"Border leafs missing loopbacks: {sorted(missing)}"
...
async def test_04_arista_fabric_is_untouched(
    ...
    assert counts == expected, (
        f"Generating {vendor_state['topology_name']} changed the {c.DC_ARISTA_NAME} fabric.\n"
        f"  Expected: {expected}\n  Found:    {counts}"
    )
...
async def test_05_management_addresses_do_not_collide(
    ...
    assert vendor_state["role_counts"]
