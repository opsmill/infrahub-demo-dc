...
async def test_definitions_imported(async_client_main: InfrahubClient, kind: str, names: list[str]) -> None:
    ...
    missing = sorted(set(names) - found)
    assert not missing, (
        f"{kind} entries declared in .infrahub.yml but not imported: {missing}.\n"
        f"  Imported: {sorted(found)}\n"
        f"  The repository reports itself in sync, so these failed to import individually."
    )
