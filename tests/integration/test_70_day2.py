...
async def test_01_edit_a_device_on_a_branch(
    ...
    day2_state["device_name"] = target.name.value
    day2_state["original_description"] = target.description.value
    ...
    assert in_main.description.value == day2_state["original_description"], (
...
async def test_02_edit_is_reviewable_and_regenerates_artifacts(
    ...
    pc_id = await h.create_proposed_change(
        client, name=f"Day two: edit {day2_state['device_name']}", source_branch=EDIT_BRANCH
    )
    day2_state["pc_id"] = pc_id
    ...
    day2_state["validators"] = validators
...
async def test_03_edit_lands_in_main(async_client_main: InfrahubClient, day2_state: dict[str, Any]) -> None:
    """Merging the edit updates the device in ``main``."""
    client = async_client_main

    await h.merge_proposed_change(client, pc_id=day2_state["pc_id"], validators=day2_state["validators"])

    device = await client.get(kind="DcimDevice", name__value=day2_state["device_name"], branch="main")
    assert device.description.value == NEW_DESCRIPTION, (
        f"{day2_state['device_name']} still reads {device.description.value!r} in main after the merge."
    )
    assert device.os_version.value == NEW_OS_VERSION, (
        f"{day2_state['device_name']} still reads OS {device.os_version.value!r} in main after the merge."
    )
...
async def test_04_move_deployment_to_a_larger_design(
    ...
    day2_state["topology_id"] = topology.id
    day2_state["devices_before"] = {
    ...
    assert day2_state["devices_before"], f"No devices in {c.DC_ARISTA_NAME} on {SCALE_BRANCH} before scaling out."
...
async def test_05_generator_grows_the_fabric(
    ...
    task = await h.run_generator(
        ...
        node_ids=[day2_state["topology_id"]],
        branch=SCALE_BRANCH,
    )
    ...
    before: dict[str, str] = day2_state["devices_before"]
    ...
    day2_state["added"] = sorted(set(after) - set(before))
    log.info("Scale-out added %d device(s): %s", len(day2_state["added"]), day2_state["added"])
...
async def test_06_new_devices_are_fully_provisioned(
    ...
    """
    client = async_client_main
    added: list[str] = day2_state["added"]
    ...
    missing = [
        f"{device.name.value}/{loopback}"
        for device in routing
        for loopback in ("loopback0", "loopback1")
        if (device.id, loopback) not in named
    ]
    assert not missing, f"New routing devices missing loopbacks: {sorted(missing)}"
