...
"""Re-exported from :mod:`tests.integration.constants`, the single home of the repository root."""
...
async def wait_for(
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            # Not transient: these are bugs in the check itself, and the most likely one here is an
            # SDK attribute or signature that a version bump renamed -- precisely what this suite is
            # meant to catch. Left to the clause below they are retried until the timeout, so a fault
            # that could be reported in a second instead burns the caller's whole budget (up to
            # GENERATOR_TIMEOUT, 40 minutes) and then reports itself as a timeout rather than as the
            # AttributeError it is. KeyError and IndexError stay transient: a partially populated
            # response from a deployment that is still starting produces those legitimately.
            raise
...
async def failing_check_details(client: InfrahubClient, validators: list[Any], limit: int = 6) -> str:
    ...
    for validator in validators:
        ...
        # Filter first, then cap. Slicing `checks[:limit]` before the conclusion test spent the
        # budget on whichever checks happened to come back first, so a validator whose first `limit`
        # checks passed reported no failing check at all -- silently omitting the one thing this
        # function exists to print.
        reported = 0
        for peer in checks:
            if reported >= limit:
                lines.append(f"      (further checks not shown; {len(checks)} in total)")
                break
...
async def assert_loopbacks_present(
    client: InfrahubClient,
    devices: list[Any],
    branch: str,
    subject: str,
    names: tuple[str, ...] = ("loopback0", "loopback1"),
) -> None:
    """Assert every device carries the expected virtual loopback interfaces.

    Args:
        client: Client bound to the deployment under test.
        devices: Devices whose loopbacks must all exist.
        branch: Branch to query.
        subject: How to name the device set in the failure message.
        names: Loopback interface names each device must carry.
    """
    loopbacks = await client.filters(
        kind="InterfaceVirtual",
        device__ids=[device.id for device in devices],
        branch=branch,
    )
    named = {(loopback.device.id, loopback.name.value) for loopback in loopbacks}
    missing = [f"{device.name.value}/{name}" for device in devices for name in names if (device.id, name) not in named]
    assert not missing, f"{subject} missing loopbacks: {sorted(missing)}"
