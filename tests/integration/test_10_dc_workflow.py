...
def workflow_state() -> dict[str, Any]:
    """IDs and counts carried between the ordered steps of this workflow.
...
async def test_02_load_design(
    ...
    workflow_state["topology_id"] = topology.id
...
async def test_03_trigger_rule_builds_fabric(
    ...
    workflow_state["role_counts"] = counts
...
async def test_04_fabric_is_complete(
    ...
    counts: dict[str, int] = workflow_state["role_counts"]
...
async def test_05_generator_rerun_is_idempotent(
    ...
    task = await h.run_generator(
        ...
        node_ids=[workflow_state["topology_id"]],
        branch=BRANCH,
    )
    ...
    assert after == workflow_state["role_counts"], (
        f"Re-running create_dc changed the fabric.\n  Before: {workflow_state['role_counts']}\n  After:  {after}"
    )
...
async def test_07_proposed_change_validators_complete(
    ...
    workflow_state["validator_nodes"] = validators


@pytest.mark.dependency(name="dc_merged", depends=["dc_validated"], scope="session")
async def test_08_merge(async_client_main: InfrahubClient, workflow_state: dict[str, Any]) -> None:
    """Merging the proposed change moves the fabric into ``main``."""
    await h.merge_proposed_change(
        async_client_main,
        pc_id=workflow_state["pc_id"],
        validators=workflow_state["validator_nodes"],
    )


@pytest.mark.dependency(depends=["dc_merged"], scope="session")
async def test_09_fabric_present_in_main(async_client_main: InfrahubClient, workflow_state: dict[str, Any]) -> None:
    ...
    assert counts == workflow_state["role_counts"], (
        f"Fabric differs after merge.\n  On {BRANCH}: {workflow_state['role_counts']}\n  On main:  {counts}"
    )
